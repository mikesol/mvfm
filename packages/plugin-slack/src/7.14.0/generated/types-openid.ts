// AUTO-GENERATED by scripts/codegen.ts — do not edit

import type { CExpr } from "@mvfm/core";
import type { OpenIDConnectTokenResponse, OpenIDConnectUserInfoResponse } from "@slack/web-api";

export interface SlackMethodsOpenid {
  connect: {
    token<A>(params: A): CExpr<OpenIDConnectTokenResponse, "slack/openid_connect_token", [A]>;
    userInfo<A = void>(
      params?: A,
    ): CExpr<OpenIDConnectUserInfoResponse, "slack/openid_connect_userInfo", [A]>;
  };
}
