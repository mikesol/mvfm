// AUTO-GENERATED by scripts/codegen.ts — do not edit

import type { CExpr } from "@mvfm/core";
import { isCExpr, makeCExpr } from "@mvfm/core";
import type { SlackMethodsAdminConversations } from "./types-admin-conversations";

function liftArg(value: unknown): unknown {
  if (isCExpr(value)) return value;
  if (typeof value === "string") return value;
  if (typeof value === "number") return value;
  if (typeof value === "boolean") return value;
  if (value === null || value === undefined) return value;
  if (Array.isArray(value)) {
    return makeCExpr("slack/array", value.map(liftArg));
  }
  if (typeof value === "object") {
    const pairs: unknown[] = [];
    for (const [k, v] of Object.entries(value as Record<string, unknown>)) {
      pairs.push(k, liftArg(v));
    }
    return makeCExpr("slack/record", pairs);
  }
  return value;
}

const mk = makeCExpr as <O, Kind extends string, Args extends readonly unknown[]>(
  kind: Kind,
  args: readonly unknown[],
) => CExpr<O, Kind, Args>;

export function buildSlackAdminConversations(): SlackMethodsAdminConversations {
  return {
    ekm: {
      listOriginalConnectedChannelInfo(params?) {
        if (params != null) return mk("slack/admin_conversations_ekm_listOriginalConnectedChannelInfo", [liftArg(params)]);
        return mk("slack/admin_conversations_ekm_listOriginalConnectedChannelInfo", []);
      },
    },
    restrictAccess: {
      addGroup(params) {
        if (params != null) return mk("slack/admin_conversations_restrictAccess_addGroup", [liftArg(params)]);
        return mk("slack/admin_conversations_restrictAccess_addGroup", []);
      },
      listGroups(params) {
        if (params != null) return mk("slack/admin_conversations_restrictAccess_listGroups", [liftArg(params)]);
        return mk("slack/admin_conversations_restrictAccess_listGroups", []);
      },
      removeGroup(params) {
        if (params != null) return mk("slack/admin_conversations_restrictAccess_removeGroup", [liftArg(params)]);
        return mk("slack/admin_conversations_restrictAccess_removeGroup", []);
      },
    },
    archive(params) {
      if (params != null) return mk("slack/admin_conversations_archive", [liftArg(params)]);
      return mk("slack/admin_conversations_archive", []);
    },
    bulkArchive(params) {
      if (params != null) return mk("slack/admin_conversations_bulkArchive", [liftArg(params)]);
      return mk("slack/admin_conversations_bulkArchive", []);
    },
    bulkDelete(params) {
      if (params != null) return mk("slack/admin_conversations_bulkDelete", [liftArg(params)]);
      return mk("slack/admin_conversations_bulkDelete", []);
    },
    bulkMove(params) {
      if (params != null) return mk("slack/admin_conversations_bulkMove", [liftArg(params)]);
      return mk("slack/admin_conversations_bulkMove", []);
    },
    convertToPrivate(params) {
      if (params != null) return mk("slack/admin_conversations_convertToPrivate", [liftArg(params)]);
      return mk("slack/admin_conversations_convertToPrivate", []);
    },
    convertToPublic(params) {
      if (params != null) return mk("slack/admin_conversations_convertToPublic", [liftArg(params)]);
      return mk("slack/admin_conversations_convertToPublic", []);
    },
    create(params) {
      if (params != null) return mk("slack/admin_conversations_create", [liftArg(params)]);
      return mk("slack/admin_conversations_create", []);
    },
    delete(params) {
      if (params != null) return mk("slack/admin_conversations_delete", [liftArg(params)]);
      return mk("slack/admin_conversations_delete", []);
    },
    disconnectShared(params) {
      if (params != null) return mk("slack/admin_conversations_disconnectShared", [liftArg(params)]);
      return mk("slack/admin_conversations_disconnectShared", []);
    },
    getConversationPrefs(params) {
      if (params != null) return mk("slack/admin_conversations_getConversationPrefs", [liftArg(params)]);
      return mk("slack/admin_conversations_getConversationPrefs", []);
    },
    getCustomRetention(params) {
      if (params != null) return mk("slack/admin_conversations_getCustomRetention", [liftArg(params)]);
      return mk("slack/admin_conversations_getCustomRetention", []);
    },
    getTeams(params) {
      if (params != null) return mk("slack/admin_conversations_getTeams", [liftArg(params)]);
      return mk("slack/admin_conversations_getTeams", []);
    },
    invite(params) {
      if (params != null) return mk("slack/admin_conversations_invite", [liftArg(params)]);
      return mk("slack/admin_conversations_invite", []);
    },
    lookup(params) {
      if (params != null) return mk("slack/admin_conversations_lookup", [liftArg(params)]);
      return mk("slack/admin_conversations_lookup", []);
    },
    removeCustomRetention(params) {
      if (params != null) return mk("slack/admin_conversations_removeCustomRetention", [liftArg(params)]);
      return mk("slack/admin_conversations_removeCustomRetention", []);
    },
    rename(params) {
      if (params != null) return mk("slack/admin_conversations_rename", [liftArg(params)]);
      return mk("slack/admin_conversations_rename", []);
    },
    search(params?) {
      if (params != null) return mk("slack/admin_conversations_search", [liftArg(params)]);
      return mk("slack/admin_conversations_search", []);
    },
    setConversationPrefs(params) {
      if (params != null) return mk("slack/admin_conversations_setConversationPrefs", [liftArg(params)]);
      return mk("slack/admin_conversations_setConversationPrefs", []);
    },
    setCustomRetention(params) {
      if (params != null) return mk("slack/admin_conversations_setCustomRetention", [liftArg(params)]);
      return mk("slack/admin_conversations_setCustomRetention", []);
    },
    setTeams(params) {
      if (params != null) return mk("slack/admin_conversations_setTeams", [liftArg(params)]);
      return mk("slack/admin_conversations_setTeams", []);
    },
    unarchive(params) {
      if (params != null) return mk("slack/admin_conversations_unarchive", [liftArg(params)]);
      return mk("slack/admin_conversations_unarchive", []);
    },
  };
}
