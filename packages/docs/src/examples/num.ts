...
const examples: Record<string, NodeExample> = {
  "num/add": {
    description:
      "Add two numbers via the semiring typeclass",
...
await foldAST(defaults(app), injectInput(prog, { x: 50 }));`,
  ...
  "num/mul": {
    description:
      "Multiply two numbers via the semiring typeclass",
    code: `const app = mvfm(prelude, console_);
const prog = app({ x: "number" }, ($) => {
...
await foldAST(defaults(app), injectInput(prog, { x: 17 }));`,
  ...
  "num/compare": {
    description:
      "Three-way comparison returning -1, 0, or 1 via the ord typeclass",
    code: `const app = mvfm(prelude, console_);
...
await foldAST(defaults(app), injectInput(prog, { x: 30, y: 75 }));`,
  ...
  "num/eq": {
    description:
      "Numeric equality — compares two numbers via the eq typeclass",
    code: `const app = mvfm(prelude, console_);
const prog = app({ x: "number", y: "number" }, ($) => {
