...
describe("redis interpreter: string commands", () => {
  ...
  it("yields SET command variants", async () => {
    ...
    expect((await run(app($.redis.set("mykey", "myvalue", "PX", 5000)))).captured[0].args).toEqual([
      "mykey",
      "myvalue",
      "PX",
      5000,
  ...
  it("yields INCR/INCRBY/DECR/DECRBY", async () => {
    ...
    expect((await run(app($.redis.incrby("counter", 5)))).captured[0].args).toEqual(["counter", 5]);
    expect((await run(app($.redis.decr("counter")))).captured[0].command).toBe("DECR");
    expect((await run(app($.redis.decrby("counter", 3)))).captured[0].args).toEqual(["counter", 3]);
  });
  ...
  it("yields MGET/MSET/APPEND/GETRANGE/SETRANGE", async () => {
    ...
    expect((await run(app($.redis.append("mykey", "extra")))).captured[0].command).toBe("APPEND");
    expect((await run(app($.redis.getrange("mykey", 0, 5)))).captured[0].command).toBe("GETRANGE");
